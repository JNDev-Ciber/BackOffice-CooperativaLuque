...
import {
  fetchElectrityPlans,
  rpcCreateElectrityPlan,
  rpcUpdateElectrityPlan,
  rpcDeleteElectrityPlan,
} from "../services/electrityService";
...
export default function ElectricityPlansPage() {
  ...
  async function load() {
    ...
    setLoading(true);
    try {
      setPlans(await fetchElectrityPlans());
    } catch (e) {
  ...
  async function onSubmit(e) {
    ...
    try {
      if (!editingItem) {
        await rpcCreateElectrityPlan(payload);
      } else {
        await rpcUpdateElectrityPlan(editingItem.id, payload);
      }
      resetForm();
      await load();
    } catch (e2) {
  ...
  async function confirmDelete() {
    ...
    setLoading(true);
    try {
      await rpcDeleteElectrityPlan(deleteTarget.id);
      await load();
    } catch (e) {
  ...
  return (
    <div style={{ maxWidth: 1600, margin: "0 auto", textAlign: "left" }}>
      <div style={{ display: "flex", alignItems: "center", marginBottom: 16 }}>
        <h2 style={{ margin: 0 }}>Planes de Electricidad</h2>
      </div>
